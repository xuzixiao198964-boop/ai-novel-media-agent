...
def main():
    ...
    try:
        ...
        # 1. 上传修复后的database.py
        print("="*60)
        print("上传修复后的database.py")
        print("="*60)
        sftp = ssh.open_sftp()
        sftp.put("E:/work/ai-novel-media-agent/backend/app/database.py",
                 "/opt/ai-novel-media-agent/backend/app/database.py")
        sftp.close()
        print("上传完成")
